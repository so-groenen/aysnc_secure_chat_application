...
ChatSession::ChatSession(IAsyncSocket_shrd_ptr socket, std::shared_ptr<IChatRoom> room)
    : m_client_socket(std::move(socket)), 
    m_hand_shaker{dynamic_pointer_cast<IPrivateBroadcaster>(room), this}, //dynamic_cast<IControllableBroadcaster*>(this)},
    m_timer(m_client_socket->get_executor()), m_room(room),
        m_ip{m_client_socket->get_ip()}
{
    m_timer.expires_at(std::chrono::steady_clock::time_point::max());
}
awaitable<size_t> ChatSession::async_read_password(std::string& buffer, size_t pass_buff_len, std::string_view delim)
{
    size_t n = co_await  m_client_socket->async_read_until(buffer, pass_buff_len, delim);
    co_return n;
}
...
awaitable<void> ChatSession::reader()
{
    ...
    { 
        ...
        {
            size_t n = co_await  m_client_socket->async_read_until(read_msg, m_read_buff_len, m_delim);
...
awaitable<void> ChatSession::writer()
{
    try
    {
        while(m_client_socket->is_open())
        {
            ...
            {
                co_await m_client_socket->async_write(m_write_msgs.front());
...
{
    std::println("[SESSION TERMINATED]");
}