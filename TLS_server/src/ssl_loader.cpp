...
auto build_security_bundle(const fs::path& certificate_dir, std::string_view root_CA_subdir, bool should_log) -> std::expected<SecurityBundle, std::string>
{
    ...
    for(auto& entry : fs::directory_iterator{certificate_dir})
    {
        ...
        {
            if(entry.path().filename().compare(root_CA_subdir) == 0)
            {
                client_root_dir_path = std::move(entry.path());
            }
        }
    ...
    if(should_log)
    {
        std::println("Found private key: {}", server_private_key_path.c_str());
        std::println("Found public key: {}", server_public_key_path.c_str());
    }
    ...
    for(auto& dir_entry : fs::directory_iterator{client_root_dir_path})
    {
        if(is_certificate(dir_entry))
        {
            if(should_log)
            {
                std::println("Found root CA: {}", dir_entry.path().c_str());
            }
